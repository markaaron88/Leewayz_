...
app.get("/searchresults", async function (req, res) {
  ...
  let status = await connection.query(sql,city, function (error, rows, fields) {
    if (error) {
      ...
      res.render("searchresults", { title: 'search', data: rows ,"menuItem" : buildLogin(req) })
    } // else
  }); // query
});
...
app.get('/listings', async function (req, res, next) {
  ...
  let status = await connection.query(sql, function (err, rows, fields) {
    if (err) throw err;
    res.render('listings', {
      title: 'names', "menuItem" : buildLogin(req),
      data: rows
    })
  }); // query
});
...
app.get("/profileresults", async function (req, res) {
  ...
  let status = await connection.query(sql,city, function (error, rows, fields) {
    if (error) {
      ...
      res.render("profileresults", { title: 'search', data: rows , "menuItem" : buildLogin(req)})
    } // else
  }); // query
});
...
app.post("/viewlisting", async function(req, res) {
  ...
  if (validation == false || validation == undefined) {
    ...
    res.render("viewlisting", {"menuItem": buildLogin(req), "listing": listing[0]});
  }
});
...
app.post("/signup", async function (req, res) {
  ...
  query = "INSERT INTO leewayz.users (firstName, lastName, email, username, password, type, isAdmin) VALUES (?, ?, ?, ?, ?, ?, ?)"

  let search = await connection.query(query, [firstName, lastName, email, username, passwd, type,
    false], function (err, rows, fields) {
      if (err) throw err;
      let id = rows.insertId
      res.render("signupsuccess", {
        query: query , "id": id, "menuItem" : buildLogin(req),
      });
    });//connect
}); //route
...
app.post("/renterprofile", async function (req, res) {
  ...
  query = "INSERT INTO leewayz.renter (userid,city, zipcode,refnames, description, numpeople,length, pets, smoking) VALUES (?,?, ?, ?, ?, ?, ?,?,?)"
  let search = await connection.query(query, [userid,city, zipcode,refnames, description, numpeople,length, pets, smoking
...
app.post("/profilesuccess", upload.single("image"), async (req, res) => {
   ...
   await connection.query(query, [data.propic,id], function (err, rows, fields) {
      if (err) throw err;
...
app.post("/addlisting", async function (req, res) {
  ...
  query = "INSERT INTO leewayz.listings(streetAddress,state,city,zipcode,owner,rooms,sqft,bathrooms,pets,dateAvailable,cooling,heating,parking,deposit,fees,utilities,description,propertyType,handicapAccessible,smokingArea) VALUES (?,?, ?, ?, ?, ?, ?,?,?,?,?,?,?, ?, ?, ?, ?, ?,?,?)"

  let search = await connection.query(query, [streetAddress,state, city, zipcode,owner, rooms, sqft, bathrooms,pets, dateAvailable,cooling,heating,parking,deposit,fees,utilities,description,propertyType,handicapAccessible,smokingArea
    ], function (err, rows, fields) {
      if (err) throw err;
      console.log("Insert execution: ", rows, fields);
      let id = rows.insertId;
...
app.post('/listingsuccess', async function(req, res){
    upload2(req,res, async function(err) {
        ...
        query = "UPDATE leewayz.listings SET image1 = ?, image2 = ?, image3 = ? WHERE id = ? ";
        await connection.query(query, [file1,file2,file3,id], function (err, rows, fields) {
      if (err) throw err;
...
app.post("/insertuser", async function(req, res) {
  ...
  try {
    let result = await insertUser(firstName, lastName, email, username, passwd, type, isAdmin);
    res.render("admin", {
      "menuItem": buildLogin(req),
    });
  }
...
app.post("/insertlisting", async function (req, res) {
  ...
  try {
    let result = await insertListing(address, state, city, zipcode, owner, rooms, sqft, bath, pets, date, cooling, heating, parking, deposit, fees, utilities, desc, type, accessibility, smoking);
    res.render("admin", {
      "menuItem": buildLogin(req),
    });
  }
  ...
  if (validation == false || validation == undefined) {
    ...
    try {
      ...
      res.render("ownedlistings", {
        "menuItem": buildLogin(req), "data": data
      });
    } catch (err) {
      console.log(err);
      res.render("admin", {
...
app.post("/contactrenter", async function(req, res) {
  ...
  if (validation == false || validation == undefined) {
    ...
    try {
      ...
      res.render("contactrenter", {"menuItem": buildLogin(req), "userInfo": user[0], "renter": renterInfo[0]});
    } catch (err) {
      console.log(err);
...
function uploadImagesListing(file1, file2, file3, id) {
  let sql = "UPDATE leewayz.listings SET image1 = ?, image2 = ?, image3 = ? WHERE id = ?";
...
function insertUser(firstname, lastname, email, username, password, type, adminStatus) {
  let sql = "INSERT INTO leewayz.users (firstName, lastName, email, username, password, type, isAdmin) VALUES (?, ?, ?, ?, ?, ?, ?)";
  let sqlParams = [firstname, lastname, email, username, password, type, adminStatus];
  return new Promise(function (resolve, reject) {
    connection.query(sql, sqlParams, function (err, rows, fields) {
      if (err) throw err;
      resolve(rows);
...
function insertListing(streetAddr, state, city, zip, owner, rooms, sqft, bath, pets, date, cooling, heating, parking, deposit, fees, utilities, desc, propType, handicapAccess, smokingArea) {
  let sql = "INSERT INTO leewayz.listings (streetAddress,state,city,zipcode,owner,rooms,sqft,bathrooms,pets,dateAvailable,cooling,heating,parking,deposit,fees,utilities,description,propertyType,handicapAccessible,smokingArea) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
  let sqlParams = [streetAddr, state, city, zip, owner, rooms, sqft, bath, pets, date, cooling, heating, parking, deposit, fees, utilities, desc, propType, handicapAccess, smokingArea];
  return new Promise(function (resolve, reject) {
    connection.query(sql, sqlParams, function (err, rows, fields) {
      if (err) throw err;
      resolve(rows);
